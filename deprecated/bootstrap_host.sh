...
sudo mount -o noatime,compress=zstd:3,subvol=@host "$PART_ROOT" "$TARGET_MNT"

sudo mkdir -p "$TARGET_MNT/boot"
sudo mount "$PART_BOOT" "$TARGET_MNT/boot"

echo "[*] Bootstrapping core developer tools onto host..."
sudo pacstrap -K "$TARGET_MNT" \
...
sudo genfstab -U "$TARGET_MNT" >> "$TARGET_MNT/etc/fstab"
...
echo "${STUDENT_LOGIN}-host" > /etc/hostname
